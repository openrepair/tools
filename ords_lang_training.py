...
def format_path_out(filename, ext="csv"):
...
def dump_data(sample=0.3, minchars=12, maxchars=65535):
    ...
    df_train.to_csv(format_path_out("ords_lang_data_training"), index=False)
    df_valid.to_csv(format_path_out("ords_lang_data_validation"), index=False)


# Experiment with classifier/vectorizer.
def experiment():
    data = pd.read_csv(format_path_out("ords_lang_data_training"), dtype=str).dropna()
    ...
    data.to_csv(format_path_out("ords_lang_results_training_tests"), index=False)

    # Save prediction misses.
    misses = data[(data["language"] != data["prediction"])]
    misses.to_csv(format_path_out("ords_lang_misses_training_tests"), index=False)
...
def do_training():
    ...
    # Save predictions to 'out' directory in csv format.
    data.loc[:, "prediction"] = predictions
    data.to_csv(format_path_out("ords_lang_results_training"), index=False)

    # Save prediction misses.
    misses = data[(data["language"] != data["prediction"])]
    misses.to_csv(format_path_out("ords_lang_misses_training"), index=False)


def do_validation(pipeline=True):
    data = pd.read_csv(format_path_out("ords_lang_data_validation"), dtype=str).dropna()
    column = data.sentence
    labels = data.language
    ...
    data.loc[:, "prediction"] = predictions
    data.to_csv(format_path_out("ords_lang_results_validation"), index=False)

    # Prediction misses for inspection.
    misses = data[(data.language != data["prediction"])]
    misses.to_csv(format_path_out("ords_lang_misses_validation"), index=False)
...
def do_detection(pipeline=True):
    ...
    data.to_csv(format_path_out("ords_lang_results_detection"), index=False)
    return data
...
def misses_report(type):
    ...
    df_in = pd.read_csv(
        pathfuncs.OUT_DIR + "/ords_lang_misses_{}.csv".format(type),
        dtype=str,
    ...
    df_out.to_csv(
        pathfuncs.OUT_DIR + "/ords_lang_misses_{}_ids.csv".format(type), index=False
    )
...
def short_misses_report():
    ...
    df_out.to_csv(pathfuncs.OUT_DIR + "/ords_lang_misses_short.csv", index=False)
    logger.debug("misses short: {}".format(len(df_out.index)))


def get_pipefile():
    return format_path_out("ords_lang_obj_tfidf_cls", "joblib")


def get_clsfile():
    return format_path_out("ords_lang_obj_cls", "joblib")


def get_tdffile():
    return format_path_out("ords_lang_obj_tdif", "joblib")
