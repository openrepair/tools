...
class DbFuncsTestCase(unittest.TestCase):
    ...
    def test_dbfuncs(self):
        ...
        sql = """SELECT * FROM `{}` WHERE `title`='Title 1'""".format(
            self.tablename)
        result = dbfuncs.query_fetchall(sql)
        ...
        vals = [('Title Two', 'desc two'), ('Title Three',
                                            'desc three'), ('Title Four', 'desc four')]
        ...
        sql = """UPDATE `{}` SET `title`='Title 2' WHERE id=2""".format(
            self.tablename)
        result = dbfuncs.execute(sql)
        self.assertEqual(1, result, 'Wrong number of rows')
        sql = """SELECT * FROM `{}` WHERE `title`='Title 2'""".format(
            self.tablename)
        result = dbfuncs.query_fetchall(sql)
        self.assertEqual(1, len(result), 'Wrong number of records')

        vals = [('Title 3', 3), ('Title 4', 4)]
        sql = """UPDATE `{}` SET `title`=%s WHERE id=%s""".format(
            self.tablename)
        result = dbfuncs.executemany(sql, vals)
