...
def fit_item_types():
    ...
    for n in range(0, len(categories)):
        ...
        dfc.sort_values(
            by=["idx", "term"], ascending=True, inplace=True, ignore_index=True
        )
    ...
    dfs = dfx.groupby(["term"]).size().reset_index(name="records")
    dfs.sort_values(by=["records"], ascending=False, inplace=True, ignore_index=True)
